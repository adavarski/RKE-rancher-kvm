...
create_vm () {
  ...
  sudo qemu-img create -f qcow2 /var/lib/libvirt/images/$SRV_HOSTNAME_PREFIX-$VM_NB.qcow2 7G

  echo "Creating virtual machine and running installer..."
  sudo virt-install --name $SRV_HOSTNAME_PREFIX-$VM_NB \
    --description 'Centos 7 - Kubernetes '$VM_NB \
    --ram 1024 \
    --vcpus 1 \
    --disk /var/lib/libvirt/images/$SRV_HOSTNAME_PREFIX-$VM_NB.qcow2,size=7 \
    --os-type linux \
